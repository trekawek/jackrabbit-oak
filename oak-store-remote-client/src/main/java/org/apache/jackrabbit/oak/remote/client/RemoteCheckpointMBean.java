...
public class RemoteCheckpointMBean extends AbstractCheckpointMBean  {

    private final RemoteNodeStore store;

    public RemoteCheckpointMBean(RemoteNodeStore store) {
        this.store = store;
    }
    ...
    protected void collectCheckpoints(TabularDataSupport tab) throws OpenDataException {
        for (String id : store.checkpoints()) {
            Map<String, String> info = store.checkpointInfo(id);
